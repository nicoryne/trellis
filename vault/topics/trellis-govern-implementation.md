...
- **Rest state — color, not grey (revised 2026-05-15)**: stylesheet `background-color` now reads `NODE_COLORS[el.data('nodeType')]` at rest with opacity 0.9 + colored shadow (blur 6, opacity 0.5). Prior rest state was uniform `#3a3f47` grey "constellation" — the team graph at rest now reads as a vibrant per-type-colored map rather than a flat grey grid. **Spotlight on hover/selection** inverts: non-spotlight nodes get an **inline grey override** (`background-color: #3a3f47`, `shadow-color: #3a3f47`, `background-opacity: 0.5`, `text-opacity: 0`) so the spotlight + neighbors keep their stylesheet-default colors and read as the focused cluster. Same treatment as [[trellis-capture-implementation|personal graph]] — visual parity across the two graph views.
- **Degree-scaled node sizing** (revised 2026-05-15): now uses the shared `recomputeNodeRadii(cy)` helper from `lib/graphDiff.ts` (previously inline). Sizes are stored as **node `data('size')`** and read by the stylesheet via `el.data('size') ?? 32` — not inline style. This survives the `removeStyle()` calls during hover-spotlight cleanup, which previously collapsed every node back to the 32 px default after the first hover-out. Hub (classification) nodes get a fixed 44 px; all others interpolate 32–80 px by degree.
